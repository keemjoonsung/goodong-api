...
public class GeneratePostMetadataAdapter implements GeneratePostMetadataPort {
    ...
    public PostAttributes generatePost(MultipartFile filePng) throws IOException {
        ...
        UserMessage multiModalUserMessage = new UserMessage("""
                ...
                The title must not contain commas and must be a single word and must be only lower-case and title must not overlap with the tags and must represent the whole image of model in the best possible way.
                And you must not contain any line break words in title , description, tags
                Additionally, you must provide the description of the model in the format `title|description|tagEntities`,
                """,
